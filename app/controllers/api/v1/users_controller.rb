class Api::V1::UsersController < ApplicationController
  skip_before_action :authenticate_request!, except: [:show, :update, :activation_request, :verify_session, :biggest_plan]
  ...
  # User sends the txid on the landing page, pre inscription purposes only
  def activation_request
    render json: EmailNotification.activation_request(@current_user,params[:txid]), status: :ok
  end
  ...
  # disable accounts if expiration is bigger than 15 days
  def disable
    result = Subscription.where(plan_id:1)
      .where(subscription_status_id: 11)
      .map { |x| 
      x.user.update(active:false) if x.expiration < Time.zone.now
      x.update(subscription_status_id:12) if x.expiration < Time.zone.now
    }.compact.count
